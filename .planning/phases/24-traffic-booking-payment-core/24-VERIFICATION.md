...
human_verification:
  - test: "Cloudflare WAF/rate-limit/challenge/block rules 실제 Zone 반영 확인"
    ...
    why_human: "Cloudflare zone/ruleset은 현재 계정에서 zone 조회가 비어 코드베이스만으로 활성 상태 판정 불가"
...
| `TRAF-02` | 24-05/08/20 | WAF/rate-limit/challenge/block | ? NEEDS HUMAN | app-layer 구현/테스트 및 runbook 있음, Cloudflare zone 실제 룰 활성 증거 미확정 |
...
**Why human:** 현재 인증 계정 기준 zone list가 비어 자동 판정 불가
...
코드 내부 구현/테스트 기준의 BLOCKER gap은 확인되지 않았다. 다만 success criteria #2(Cloudflare 실 zone 적용)와 #5(실 Toss sandbox round-trip)의 외부 운영 증거가 미완료라 `status: human_needed`를 유지한다.

---

_Verified: 2026-05-10T13:27:50Z_  
