...
| Validation Backfill | PREF-02 | ACCEPTED_RISK | `.planning/phases/22-preflight-closure/22-VALIDATION-BASELINE.md#baseline-summary` | 2026-05-04 KST | Maintainer + Operator | Source baseline has no remaining `BLOCKER` rows, but six `ACCEPTED_CAVEAT` rows remain visible and are not counted as `PASS`. | Collect direct evidence for accepted caveats before production launch/significant traffic or if a related launch-readiness incident occurs. |
| Hardening | PREF-03 | ACCEPTED_RISK | `.planning/phases/22-preflight-closure/22-HARDENING-REGISTER.md#register` | 2026-05-04 KST | Maintainer + Operator | Source register has no remaining `BLOCKER` findings, but Valkey, R2, SMS, Email, and Legal hardening gaps are accepted risks rather than green evidence. | Collect direct provider/operator evidence before production launch/significant traffic or if a related operational incident occurs. |
