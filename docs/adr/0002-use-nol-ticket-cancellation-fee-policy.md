...
- For Full Reservation Cancellation, provider `CANCELED` or zero remaining balance is stored locally as `CANCELED`; a positive policy-retained balance may remain `PARTIAL_CANCELED` while the Reservation and all target Ticket Items are cancelled.
- Historical Ticket Item Partial Cancellation has a different local contract: while valid Ticket Items remain, the parent Reservation stays `CONFIRMED` and its payment stays `DONE`, even if Toss reports `PARTIAL_CANCELED`. The cancelled Ticket Items, revoked QR Credentials, and provider cancellation amounts carry the partial-cancellation evidence. Do not copy the provider status into the parent payment without checking this contract; QR retrieval, active-ticket manifests and active sales queries require `DONE`.
- Reconciliation must compare the cancellation scope, completed cancellation sum, provider balance, remaining Ticket Items and their credentials. Equality of provider and local status strings is not a completion condition.
