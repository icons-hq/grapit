...
PR #232의 자동 검토 4건을 실제 실패 회귀로 확인한 뒤 수정했다. 좌석 재활성화/즉시 개방 전용 권한의 메뉴와 공연 업무 링크를 경로 권한에 맞추고 작업 버튼은 개별 권한으로 제한했다. 신규 prepare의 확정 거절은 소유 주문 조회가 JSON `null`로 미생성을 확인한 경우에만 재선택하며, 기존 주문·조회 실패·응답 유실은 원 식별자를 보존한다. Nest의 빈 null 응답도 실제 HTTP에서 재현해 명시적인 JSON으로 보완했다.

PG 자료를 포함하는 조회와 export는 최대 31일로 서버·UI에서 제한했다. 운영 이력이 없는 공연을 삭제할 때 연결 초안을 같은 transaction에서 정리하며, 예매가 있으면 둘 다 보존하고 감사 기록은 남긴다. migration 파일을 바꾸거나 운영 데이터를 직접 정리하지 않았다.

관련 실제 PostgreSQL/HTTP 47개, 고객·권한·정산·패치 기록 UI 47개, 추가 HTTP 표현 4개, 관리자 unit 38개 및 패치 기록 추가 확인 13개를 통과했다. 일부 검증 묶음은 서로 겹치므로 이 수치를 합산한 총계로 사용하지 않는다. 수정된 최종 후보의 CI와 실제 브라우저 재검증을 릴리스 증거에 추가한다.

후속 병렬 검토에서 단독 좌석 권한의 이력 API가 두 권한을 모두 요구하는 불일치를 추가 재현했다. 이력에만 명시적인 any-capability 조건을 적용하고 기존 all-capability 및 변경 API 조건을 유지했다. 예매 조회만 가진 사용자의 즉시 개방 버튼·폼·확인을 제한했으며, 즉시 개방 단독 권한에는 예매 조회 권한 필요 안내를 제공한다. 관련 HTTP/guard/표현 17개, 좌석 UI 16개, API/Web typecheck와 lint를 통과했다. 격리 브라우저에서 재활성화 단독 계정의 메뉴·공연/회차 선택·실제 이력 조회도 확인했다.
