...
- 최초 payment deadline은 서버의 공연 정책을 사용하고 prepare 시 Redis TTL을 동일 기한까지 연장한다. 결제 앱으로 넘길 때 기존 grace(8분, 생성 후 총 15분 cap)와 서버 응답 deadline을 유지한다. 클라이언트가 보낸 deadline을 기준으로 삼지 않는다.
...
1. dry-run은 READ ONLY/REPEATABLE READ다. 회차·누락 티켓 수·권리 수·해시만 출력한다.
...
| API 전체 integration | 6파일·59테스트 통과. testcontainers PostgreSQL 16/Valkey 8, 운영 DB 미사용 |
