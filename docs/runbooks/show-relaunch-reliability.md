...
| C14 | 매출/부분취소/정산 통화/CSV 회귀. 국내 매칭 PG gross/fee/net 및 상점 외화 명세 155주문·203행·지급 38,312,867원 대조. 별도 환불 362,000원은 운영자 송금 완료 확인 | 은행 거래내역·송금 원장 독립 대조, 공연별 비용 귀속. 운영자 확인과 독립 실증을 구분 |
...
2026-09-18 복구 작업은 당시의 계정·회차 일괄 입장을 유지했다. 이후 전면 개편의 [ADR 0011](../adr/0011-consume-one-ticket-item-per-field-scan.md)은 새 스캔을 해당 Ticket Item 한 장의 입장으로 변경한다. 과거 일괄 입장 결과는 되돌리지 않으며 그 수를 실제 방문자 수와 동일하다고 해석하지 않는다. 개편 적용 후에는 [좌석별 현장 절차](seat-level-field-operations.md)를 사용한다.
...
유효 티켓 매출과의 362,000원 차이는 과거 별도 송금 환불로 처리한 계좌이체 1건이다. 이전 작업의 사용자 지시에 따라 좌석 1장만 DB 취소됐고, PG는 724,000원 전체 승인/정산 및 취소 0건을 유지한다. 2026-09-18 후속 답변에서 운영자가 별도 송금 완료를 확인했다. 이는 운영자 확인이며 은행 이체 영수증을 직접 조회한 결과는 아니다. 실제 송금 시각은 미제공이므로 취소일이나 확인일로 만들어 기록하지 않았다. 추가 PG 취소·송금·상태 보정은 하지 않는다.

정산 화면은 국내 매칭 매출 차액이 0이 아니면 부호를 포함한 금액과 PG·티켓 상태, 취소 정산 시점, 별도 송금 원장 확인 안내를 표시한다. 금액을 자동 보정하거나 차이를 추가 수수료로 분류하지 않는다.

### 외화 지급 명세 후속 대조

운영자 로그인 후 토스 상점관리자에서 매출일 기준 `2026-05-01`–`2026-09-18`의 외화정산 건별·요약 원본을 다운로드해 읽기 전용으로 계산했다. 요약 29행의 각 행에서 `당일 정산액 + 이월 미입금 = 입금 정산액 + 당일 미입금`이 일치했고, 당일 정산액 합계와 지급 합계는 모두 **38,312,867원**이다. 마지막 미입금 잔액은 0원이다. 토스가 입금완료로 표시한 지급일별 금액은 7월 6일 36,372,427원, 8월 6일 1,940,440원이다. 은행 계좌 거래내역을 별도로 조회한 것은 아니다.

건별 명세 203행은 155주문이다. DB에 있는 외화 결제 153건은 승인액·취소액·잔액 모두 기존 1,190건 PG 조회 및 현재 DB와 일치했다. 미발권 ABORTED 295건·EXPIRED 45건은 승인 매출 명세 누락으로 세지 않는다.

나머지 2주문은 DB payment 행이 없지만 PG의 새 조회에서 모두 `CANCELED`, 잔액 0, 취소 `DONE`이다. 하나는 공연명을 가진 55.76 USD 테스트취소이며 로컬 예약·티켓이 없다. 다른 하나는 246.16 USD 예약 취소이며 로컬 예약은 CANCELLED, 티켓 0개다. 취소된 원금 301.92 USD를 미환불로 처리하지 않는다. 반환되지 않은 두 건의 PG 수수료 **13.28 USD**는 상점 정산 비용에 포함돼 있다. 원래 결제 행을 새로 만들거나 재환불하지 않았다.

| 외화 명세 구분 | USD |
| --- | ---: |
| 승인 합계 | 39,271.36 |
| 취소 합계 | 11,969.36 |
| 취소 후 원금 잔액 | 27,302.00 |
| PG 수수료 | 1,780.07 |
| 환전·인출 수수료 | 523.97 |
| 외화 정산 합계 | 24,997.96 |

`27,302.00 - 1,780.07 - 523.97 = 24,997.96 USD`이며, 결제 시 원화 환산액과 실제 정산 환율을 동일하게 취급하지 않는다. 유효 외화 티켓 매출 39,768,000원과 별도로 전체 취소 4건의 보유 수수료 382,000원이 있다. 따라서 유효 티켓 매출과 PG 순지급액의 차이를 전부 PG 수수료라고 부를 수 없다.

국내 매칭 지급 154,166,390원과 **상점 기간 전체 외화 지급** 38,312,867원을 더한 참고 합계는 192,479,257원이다. 운영자 확인 별도 환불 362,000원을 차감한 계산값은 192,117,257원이다. 이 합계는 범위가 같은 공연 전용 정산액이 아니다. 외화 명세에는 로컬 예약이 없는 테스트취소 비용 2.45 USD를 포함한 DB payment 미대응 비용 13.28 USD가 있고, 공연별 비용 귀속은 아직 배분하지 않았다. PG 주문명의 공연 표기는 관련성을 보여주지만 회계 비용 배분 승인을 대신하지 않는다.

운영 정산 화면에 상점 전체 외화 지급액 38,312,867원을 입력해 표시 식을 확인했을 때 차이는 6,982,743원(`선택 공연의 유효 매출 199,462,000 - 위 참고 합계 192,479,257`)이었다. 별도 송금까지 차감한 참고값과의 차이는 7,344,743원이다. 이 둘은 **범위가 다른 자료를 입력한 참고 계산**으로만 기록하며, 공연 정산 확정·미입금·순이익·총수수료의 근거로 사용하지 않는다. 공연 정산을 확정할 때는 테스트·공연 거래의 비용 귀속 및 은행 거래내역·송금 원장을 별도로 대조해야 한다. 화면 입력은 저장하지 않는 일시적 계산이며 검증 후 비웠다.
