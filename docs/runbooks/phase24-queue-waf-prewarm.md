...
- Create or reuse a dedicated service account, for example:
  - `scheduler-prewarm@PROJECT_ID.iam.gserviceaccount.com`
...
gcloud scheduler jobs create http grabit-prewarm-scale-up \
  ...
  --uri="https://api.heygrabit.com/api/v1/internal/prewarm/services/grabit-api" \
  --http-method=POST \
  --headers="Content-Type=application/json,x-prewarm-control-token=${PREWARM_CONTROL_TOKEN}" \
  --message-body='{"minInstances":120}' \
  --oidc-service-account-email="scheduler-prewarm@${PROJECT_ID}.iam.gserviceaccount.com" \
  --oidc-token-audience="https://api.heygrabit.com/api/v1/internal/prewarm/services/grabit-api"
```
...
gcloud scheduler jobs create http grabit-prewarm-step-down \
  ...
  --schedule="10 11 * * *" \
  --time-zone="Asia/Seoul" \
  --uri="https://api.heygrabit.com/api/v1/internal/prewarm/services/grabit-api/step-down" \
  --http-method=POST \
  --headers="Content-Type=application/json,x-prewarm-control-token=${PREWARM_CONTROL_TOKEN}" \
